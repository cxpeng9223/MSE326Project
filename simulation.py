...
def generate_agent(config):
    ...
    k = config["k"]
    return ([(0, 0, 0) for _ in range(k)], new_thetas(config["types"]))
...
def select_arms(state, config, log):
    ...
    for agent in state["agents"]:
        prob = random.random()
        if prob <= config["mix"]:
            action = config["strategy"](agent[0])
            actions.append(action)
        else:
            action = config["strategy_alt"](agent[0])
            actions.append(action)
...
def update_histories(state, config, log):
    ...
    for agent, action, payoff in zip(state["agents"], state["actions"], state["payoffs"]):
        wins, total, _ = agent[0][action]
        agent[0][action] = (wins + payoff, total + 1, payoff)
    return state
